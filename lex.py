...
def p_booleanos(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('Boolean'))
...
def p_math_sw(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('Math'))
...
def p_lista_exp(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('lista'))
...
def p_trunc1(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('trunc'))
...
def p_trigonometric(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('trigonometric'))
...
def p_ext(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('extract'))
...
def p_time_type(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('time'))
...
def p_d_part(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('Date'))
...
def p_binary_string(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('Binary'))
...
def p_bs_siuw(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('Substring'))
...
def p_trim(p):
    ...
    try:
        id = inc_index()
        p[0] = id
        ast_graph.node(str(id), str('trim'))
